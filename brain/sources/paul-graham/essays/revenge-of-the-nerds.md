...
Source: https://www.paulgraham.com/icad.html
...
computing. But he went ahead and did it. That is, he compiled the eval in my paper into [IBM] 704 machine
...
idea recently reinvented as XML. When Lisp first appeared, these ideas were far
...
or just expand your programming horizons, I would learn more about macros.
...
A good example is the airline fare search program that ITA Software licenses to
...
software shifts off the desktop and onto servers (a future even
...
a supposed e-commerce standard called SET that no one now
...
But a recent article about ITA in New
...
just three months developing something new, it would be five years before you had it too.
...
In Common Lisp this would be (defun foo (n)

(lambda (i) (incf n i))) and in Perl 5, sub foo {
...
In Smalltalk the code is slightly longer than in Lisp foo: n

|s|

s := n.

^[:i| s := s+i. ] because although in general lexical variables work, you can't
...
to return values: function foo(n) {

return function (i) {

return n += i } } (To be fair, Perl also retains
...
function to return. This is what you end up with: def foo(n):
...
return bar Python users might legitimately ask why they can't

just write def foo(n):

return lambda i: return n += i or even def foo(n):

lambda i: n += i and my guess is that they probably will, one day.
...
return self.n I include these because I wouldn't want Python
...
as you can get in Java: public interface Inttoint {
...
Here is the accumulator generator in other Lisp dialects: Scheme: (define (foo n)
...
It also set off an extensive and often useful discussion on the LL1 mailing list. See particularly the mail by Anton van Straaten on semantic
