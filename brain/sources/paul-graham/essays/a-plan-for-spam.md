...
Source: https://www.paulgraham.com/spam.html
...
which I calculate as follows [1]: (let ((g (* 2 (or (gethash word good) 0)))
...
(min 1 (/ b nbad))))))))) where word is the token whose probability we're

calculating, good and bad are the hash tables

I created in the first step, and ngood and nbad are the number of nonspam and spam messages respectively.
...
the mail is spam. If probs is a list of the fifteen individual probabilities, you

calculate the combined probability thus: (let ((prod (apply #'* probs)))

(/ prod (+ prod (apply #'* (mapcar #'(lambda (x)

(- 1 x))

probs))))) One question that arises in
...
Words that occur disproportionately rarely in spam (like "though" or "tonight" or "apparently")
...
Indeed, most antispam techniques so far have been like pesticides that
...
this: Hey there. Thought you should check out the following:

http://www.27meg.com/foo because that is about as much sales pitch as content-based
...
may not decrease the amount of spam that spammers send, they can certainly help filters to decrease the amount of
...
this article. The fifteen most interesting words in this spam are: qvp0045
...
valuable 0.82347786 This time the evidence is a mix of good and bad. A word like
...
For comparison, here is an example of that rare bird, a spam that

gets through the filters. Why? Because by sheer chance it happens

to be loaded with words that occur in my actual email: perl 0.01
...
difficult 0.071706355 There are a couple pieces of good news here. First, this mail
...
Its fifteen most interesting words are as follows: continuation 0.01
...
what 0.19212411 Most of the words here indicate the mail is an innocent one.
...
a domain name is about to expire (at least, if they are the actual
